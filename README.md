...
We recommend collecting data in the Arduino Serial Monitor and converting to a .txt file, then plotting in Python. The first and second columns of the Arduino readout are the voltage (in mV) and a quantity linearly related to temperature (conversion in Python file). The Python file takes in the current passed through the sample, which gives resistance form Ohm's law. The error computation in the Python file uses reported manufacturer uncertainites and accounts for a reported 400 ms offset between temperature and voltage readouts. 
